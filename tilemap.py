...
class Tilemap:
    ...
    def render(self, m_pos):
        ...
        if self.draw_grid:
            for x in range(self.surf.get_width() // self.tile_size + 1):
                pygame.draw.line(self.surf, (200, 200, 200),
                                (x * self.tile_size, 0), (x * self.tile_size, self.surf.get_height()))
            for y in range(self.surf.get_height() // self.tile_size + 1):
                pygame.draw.line(self.surf, (200, 200, 200),
                                (0, y * self.tile_size), (self.surf.get_width(), y * self.tile_size))
